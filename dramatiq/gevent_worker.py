...
class GeventWorker:
    ...
    distribute those messages to individual greenlets for
    processing. Workers don't block the current greenlet so it's
    up to the caller to keep it alive.

    This is a gevent-compatible version of the Worker class.
    ...
    Parameters:
      ...
      worker_threads(int): The number of worker greenlets to spawn.
    """
    ...
    def start(self):
        ...
        worker_middleware = _GeventWorkerMiddleware(self)
    ...
    def pause(self):
        """Pauses all the worker greenlets.
        """
        ...
        # Wait for all to enter paused state
        for consumer in self.consumers.values():
            consumer.paused_event.wait()

        for worker in self.workers:
            worker.paused_event.wait()

    def resume(self):
        """Resumes all the worker greenlets.
        """
        for consumer in self.consumers.values():
            consumer.resume()
    ...
    def stop(self, timeout=600000):
        ...
        for greenlet in self.workers:
            greenlet.stop()

        # Join all worker greenlets with timeout
        timeout_seconds = timeout / 1000
        self._join_all(self.workers, timeout_seconds)
        ...
        # Join all consumer greenlets with timeout
        self._join_all(self.consumers.values(), timeout_seconds)
    ...
    def _add_consumer(self, queue_name, *, delay=False):
        ...
        consumer = self.consumers[queue_name] = _GeventConsumer(
            broker=self.broker,
    ...
    def _join_all(self, greenlets, timeout=None):
        """Join a list of greenlets with an optional timeout in seconds."""
        from gevent import joinall
        joinall(list(greenlets), timeout=timeout)


class _GeventWorkerMiddleware(Middleware):
...
class _GeventConsumer(Greenlet):
    def __init__(self, *, broker, queue_name, prefetch, work_queue, worker_timeout):
        super().__init__()
        self.logger = get_logger(__name__, "GeventConsumer(%s)" % queue_name)
        self.running = False
        self.paused = False
    ...
    def _run(self):
        self.logger.debug("Running consumer greenlet...")
        self.running = True
        ...
        while self.running:
            if self.paused:
                ...
                sleep(self.worker_timeout / 1000)
                continue
    ...
    def post_process_message(self, message):
        """Called by worker greenlets whenever they're done processing
        ...
        while True:
            ...
            # users who don't deploy frequently.
                ...
                sleep(POST_PROCESS_MESSAGE_RETRY_DELAY_SECS)
                continue
    ...
    def stop(self):
        """Initiate the GeventConsumer shutdown sequence.
        """
        self.logger.debug("Stopping consumer greenlet...")
        self.running = False

    def close(self):
        """Close this consumer greenlet and its underlying connection.
        """
        try:
            if self.consumer:
...
class _GeventWorker(Greenlet):
    """GeventWorker processes incoming messages off of the work queue
    on a loop. By themselves, they don't do any sort of network IO.

    Parameters:
      broker(Broker)
      consumers(dict[str, _GeventConsumer])
      work_queue(Queue)
      worker_timeout(int)
    """
    ...
    def __init__(self, *, broker, consumers, work_queue, worker_timeout):
        super().__init__()
        self.logger = get_logger(__name__, "GeventWorker")
        self.running = False
        self.paused = False
        self.paused_event = Event()
        self.broker = broker
    ...
    def _run(self):
        self.logger.debug("Running worker greenlet...")
        self.running = True
        ...
        self.logger.debug("Worker greenlet stopped.")
    ...
    def stop(self):
        """Initiate the GeventWorker shutdown process.
        """
        self.logger.debug("Stopping worker greenlet...")
        self.running = False
